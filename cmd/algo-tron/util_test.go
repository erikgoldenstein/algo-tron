...
func TestValidateJoin(t *testing.T) {
	cases := []struct {
		...
		{"reserved online from remote IP", "online", "pass", "1.2.3.4", "ERROR_NO_PERMISSION"},
		{"reserved name is case-insensitive", "Alice", "pass", "1.2.3.4", "ERROR_NO_PERMISSION"},
		{"reserved online is case-insensitive", "ONLINE", "pass", "1.2.3.4", "ERROR_NO_PERMISSION"},
...
func TestValidateBio(t *testing.T) {
	cases := []struct {
		...
		{"contact markup", "contact", "</span><script>alert(1)</script>", "ERROR_INVALID_BIO"},
		{"contact html entity", "contact", "&lt;script&gt;", "ERROR_INVALID_BIO"},
		{"source", "src", "https://github.com/erikgoldenstein/tron-bot", ""},
		{"source other host", "src", "https://gitlab.com/a/b", ""},
		{"source http", "src", "http://git.example.org/team/repo", ""},
		{"source arbitrary text", "src", "my local git repository", ""},
		{"source URL query", "src", "https://git.example.org/a/b?x=1&y=2", ""},
		{"source max length", "src", strings.Repeat("x", 48), ""},
		{"source too long", "src", "https://github.com/a/" + strings.Repeat("x", 28), "ERROR_INVALID_BIO"},
		{"source printable markup text", "src", "<script>", ""},
