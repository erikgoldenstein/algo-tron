...
function scoreRow(p, i, includeInfo = true, nameChars = scoreNameChars) {
  ...
  return '<tr data-score-key="' + esc(scoreRowKey(p)) + '"' + (followed ? ' class="followed"' : '') + '>'
    + '<td class="num">' + (i + 1) + '</td>'
    + '<td class="name" style="color:' + c + '"><span class="score-name-content"><span class="score-name-text"><span class="namestr score-hover-target score-follow-target" data-follow-name="' + esc(label) + '" data-name="' + esc(label) + '" data-username="' + esc(p.username) + '" data-version="' + esc(p.version || '') + '" data-show-version="' + (p.showVersion && p.version ? 'true' : 'false') + '" data-name-chars="' + nameCharsData + '" data-first-seen="' + (p.firstSeen || 0) + '" data-contact="' + esc(contact) + '" data-src="' + esc(src) + '" data-old-owner="' + (p.oldOwner ? 'true' : 'false') + '">' + scoreNameMarkup(p.username, p.version || '', !!p.showVersion, nameChars) + '</span></span>' + (followedDead ? ' <span class="follow-status">(currently dead)</span>' : '') + old + winner + '</span></td>'
    + info
    + '<td class="sep">|</td>'
