...
The live leaderboard contains connected human players, including passwordless sessions; internal filler bots are excluded. Passwordless sessions disappear from it on disconnect and never contribute to historical persisted period boards. `init` and `end` carry the sidebar's first page inline plus a `scoreboardHasMore` flag so the client knows whether the sidebar can paginate further; subsequent pages come through `scoreboard` messages.
