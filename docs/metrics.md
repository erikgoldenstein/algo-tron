...
- On the viewer HTTP server with Basic auth: `-view-metrics-auth user:pass`. Mounts `/metrics` on the same port as the viewer, protected by HTTP Basic auth. Works with Prometheus' [`basic_auth`](https://prometheus.io/docs/prometheus/latest/configuration/configuration/#basic_auth) scrape config and uses the viewer's TLS termination.

Both can be enabled at the same time. Setting neither disables `/metrics` entirely.

Metric definitions live in `cmd/algo-tron/metrics.go`; the code that updates them is distributed across the server. The Basic auth middleware lives in `view.go` (`basicAuth` uses `subtle.ConstantTimeCompare` so the check doesn't leak via timing). Grep for `metric` to find emit sites.
...
| Name                                        | Labels      | Meaning                                                                                  |
|---------------------------------------------|-------------|------------------------------------------------------------------------------------------|
| `tron_games_total`                          | none           | Total games finished.                                                                    |
| `tron_ticks_total`                          | none           | Total ticks processed across all games.                                                  |
| `tron_viewers_kicked_total`                 | none           | Viewers dropped because their 16-frame send buffer overflowed. Overload signal.       |
| `tron_tcp_accept_errors_total`              | none           | Errors from the TCP `Accept` loop (retried with exponential backoff up to 1s).            |
| `tron_tcp_panics_total`                     | none           | Panics recovered in per-connection bot handlers.                                          |
...
| `tron_chat_rate_limited_total`              | none           | Chat packets refused by the per-tick rate limit.                                          |
| `tron_history_rate_limited_total`           | none           | History API requests refused by the per-client rate limit.                                |
| `tron_player_disconnect_mid_game_total`     | none           | Players killed mid-game because their TCP connection went away.                           |
| `tron_bots_kicked_total`                    | none           | Bot connections dropped because their per-bot send buffer (`botSinkBuf` packets) overflowed because the bot stopped reading or its link stalled. The bot-side analog of `tron_viewers_kicked_total`. |
...
| `tron_tick_deadline_misses_total`            | none           | Ticks whose scheduler woke after the planned deadline.                                  |
| `tron_tick_processing_overruns_total`        | none           | Ticks whose processing plus fanout took at least one full tick interval.                |
| `tron_tcp_connections_total`                 | none           | TCP connection handlers started, including pre-join connections.                        |
...
| `tron_assisted_moves_total`                  | none           | Moves supplied by the server fallback after a missing/invalid move.                     |
| `tron_viewer_messages_received_total`        | none           | Messages received from viewer WebSocket clients.                                        |
| `tron_viewer_messages_queued_total`          | none           | Viewer messages successfully queued for WebSocket delivery.                             |
...
Recomputed once a minute (and at boot) by `updateDisconnectStats`, which queries the `game_participants` ledger off-lock over trailing windows. A high `top_user_share` with few users points at a single bad link; a low share spread across many users points at the server.
...
- `rate(tron_bots_kicked_total[5m]) > 0`: client reads or server writes are stalling; check `tron_bot_write_seconds`.
